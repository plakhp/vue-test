  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list" stripe border style="width: 100%">
      ...
      <el-table-column label="进度" width="120">
        <template slot-scope="scope">
          <span v-if="scope.row.auditState === 0" class="color-black">未处理</span>
          <span v-if="scope.row.auditState === 1" class="color-green">通过</span>
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      this.loading = true
      const { data: res } = await this.$http.get('profit-apply/list', { params: this.filter })
      this.loading = false
    ...
    async apply(item) {
      ...
      const { data: res } = await this.$http.patch(`profit-apply/${item.id}/approve`, readyData)
